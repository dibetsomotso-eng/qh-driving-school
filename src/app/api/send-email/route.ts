...
export async function POST(request: Request) {
  ...
  try {
    ...
    const results = await Promise.all(promises) as { data: unknown; error: { message: string; name: string } | null }[];
    const failures = results.filter((r) => r.error !== null);

    if (failures.length > 0) {
      console.error('One or more emails failed to send:', JSON.stringify(failures.map(f => f.error)));
      return NextResponse.json(
        { success: false, message: 'One or more email notifications failed to send.', errors: failures.map(f => f.error) },
        { status: 500 }
      );
    }
